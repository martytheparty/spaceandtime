...
describe('VisualizationService', () => {
  ...
  it('should create a new visualization', () => {
      ...
      expect(service.getVisualizations().length).toBeGreaterThan(0);
  ...
  it('should test upsert for empty visualization', () => {
    ...
    service.initializeExistingWithComponent(mockEmptyViz, mockComponent);

    expect(mockComponent.setAsInitialized).toHaveBeenCalled();
  ...
  it('Remove a visualization based on a id', () => {
    expect(service.visualizations.length).toBe(0);
    ...
    expect(service.visualizations.length).toBe(1);
  ...
  it('gets a visualization for a component', () => {
      ...
      const mockViz: StVisualization = {
        ...
        vizComponent: {
          ...
          stRndererInputId: () => rendererId
          
        }
      } as unknown as StVisualization;
      const mockViz1: StVisualization = { stRendererId: rendererId, stLeft: 0, stWidth: 50 } as StVisualization;
      const mockViz2: StVisualization = { stRendererId: 100,stLeft: 50, stWidth: 50 } as StVisualization;
      const mockViz3: StVisualization = { stRendererId: 200,stLeft: 100, stWidth: 50 } as StVisualization;

      const vizList: StVisualization[] = [mockViz1, mockViz2, mockViz3];

      const vizComponent: VizComponent = fixture.debugElement.children[0].componentInstance;
      const stViz: StVisualization | undefined = service.getVizForComponent(vizComponent, vizList);
